...
def _get_nested(cfg: DictConfig, dotpath: str) -> Any:
    """Resolve a dot-separated key path, unwrapping OmegaConf scalars."""
    node: Any = cfg
    for part in dotpath.split("."):
        if not OmegaConf.is_config(node) or part not in node:
            raise KeyError(dotpath)
        node = node[part]
    return OmegaConf.to_object(node) if OmegaConf.is_config(node) else node
...
def validate_config(cfg: DictConfig) -> None:
    ...
    for spec in _FIELD_SPECS:
        ...
        if spec.type_ is not None:
            expected: type[Any] | tuple[type[Any], type[Any]] = (
                (spec.type_, int) if spec.type_ is float else spec.type_
            )
        ...
        if spec.min_val is not None and value < spec.min_val:
            errors.append(
                f"  [{spec.path}] {value!r} is below minimum {spec.min_val}"
            )
        if spec.max_val is not None and value > spec.max_val:
            errors.append(
                f"  [{spec.path}] {value!r} exceeds maximum {spec.max_val}"
            )
